    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="工号">
          <el-input v-model="listQuery.filter1" placeholder="请输入工号" />
        </el-form-item>
        <el-form-item label="姓名">
          <el-input v-model="listQuery.filter2" placeholder="请输入姓名" />
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="姓名" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="角色" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="技能组信息" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="是否网络座席" prop="type3" align="center">
          <template slot-scope="{ row }">
            <el-tag :type="row.type1 == 1? 'success' : 'info'">{{ row.type1 == 1? '是' : '否' }}</el-tag>
          </template>
        </el-table-column>
