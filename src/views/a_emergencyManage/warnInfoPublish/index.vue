    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="新闻编号">
          <el-input v-model="listQuery.filter1" placeholder="请输入新闻编号" />
        </el-form-item>
        <el-form-item label="新闻标题">
          <el-input v-model="listQuery.filter2" placeholder="请输入新闻标题" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="新闻编号" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="新闻标题" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="新闻内容" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="附件信息" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
